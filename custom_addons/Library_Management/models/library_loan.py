...
class LibraryLoan(models.Model):
    ...
    def create(self, vals_list):
        for vals in vals_list:
            ...
            if not book.is_available:
                raise ValidationError(_("El libro '%s' no está disponible.") % book.name)
            
            # Validar límite de 5 préstamos
            active_loans = self.search_count([
                ('member_id', '=', vals.get('member_id')),
                ('state', 'in', ['active', 'overdue'])
            ])
            if active_loans >= 5:
                raise ValidationError(_("El socio ya tiene 5 préstamos activos."))
            
            book.is_available = False
        return super().create(vals_list)

    def action_return_book(self):
        """ Registro manual de devolución """
        for record in self:
    ...
    def action_renew_loan(self):
        """ Renovación desde el portal (solo si no está vencido) """
        self.ensure_one()
    ...
    def _cron_check_overdue_loans(self):
        """ Acción programada diaria """
        limit_date = fields.Date.today() - timedelta(days=30)
        overdue_loans = self.search([
            ('state', '=', 'active'),
            ('loan_date', '<', limit_date)
        ])
        template = self.env.ref('library_management.email_template_loan_overdue', raise_if_not_found=False)
